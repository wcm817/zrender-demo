...
export default class Table {
  ...
  setDataConfig() {
    ...
    if (flag) {
      ...
      // 基本项
      colNames = row.map((item: any) => item.name);
      metaNames = colNames.map((item: any) => ({
        field: item,
        name: item,
      }));
      ...
      let children: any = [];
      for (var i = 0; i < catCol.length; i++) {
        let item = catCol[i].name;
        let nameArr = this.data.filter((f) => f[item]).map((m) => m[item]);
        nameArr = [...new Set(nameArr)]
        if (aggrCol.length) {
          nameArr = nameArr.map((m) => {
            return {
              key: m,
              children: aggrCol.map((aItem: any) => `${aItem.legend.toLocaleLowerCase()}(${aItem.name})`)
            }
          })
        }
        children = nameArr;
      ...
      colNames.push(colNameObj);

      const allCatName = catCol.map((item: any) => item.name).join('/');
      const metaObj = {
        field: 'allCat',
        name: allCatName
      };
      metaNames.push(metaObj);
    }
